...
## Contribution Boundary Check

Before naming anything as a `contribution`, `method`, `strategy`, `framework`, or `module` in the abstract, introduction, contribution list, or conclusion, classify it first:

- **Contribution**: A paper-level claim you intend to defend with evidence. This may be a new method, dataset, system pipeline, analysis, benchmark, or evaluation finding.
- **Supporting component**: An existing method or tool integrated so the pipeline can run.
- **Implementation detail**: Training schedules, optimizer choices, thresholds, prompt wording, input resolutions, repeated training runs, post-processing switches, and similar setup decisions.
- **Constraint or limitation**: Missing labels, incomplete baselines, unavailable ablations, engineering shortcuts, or temporary assumptions.

Use these rules aggressively:

- Do **not** promote an implementation detail into a named contribution unless the paper explicitly studies it as a core idea and supports it with controlled evidence.
- If a committee member could fairly ask "`Is this your claimed method contribution, or just how you trained/ran the system?`" and the honest answer is the latter, keep it out of the contribution framing.
- For theses, **workload is not the same as contribution**. A detail may reflect real effort while still belonging in Methods or Experiments rather than the Abstract or contribution list.
- Default to the **weakest accurate label**: `training setup`, `continued optimization`, `implementation detail`, `detector configuration`, not `strategy`, `framework`, `mechanism`, or `novel module`.
- Put engineering details in Methods/Experiments unless they are necessary to interpret a headline result.
...
Before finalizing the abstract, run this gate:

1. Does every named method or contribution correspond to a paper-level claim I can defend?
2. Have I spent more words on the actual workflow and findings than on training details or setup?
3. If I mention an engineering choice, is it necessary for understanding the result rather than just recording effort?
4. Am I describing an implementation fact plainly, or am I dressing it up with words like `strategy`, `framework`, or `novel`?
